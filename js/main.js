...
(() => {
    ...
    pills.forEach((pill) => {
        pill.addEventListener("click", (e) => {
            ...
            if (target) {
                lenis === null || lenis === void 0 ? void 0 : lenis.scrollTo(target, { offset: -100 });
            }
        });
    });
    ...
    function findCartIndex(id, addons) {
        return cart.findIndex((c) => {
            if (c.id !== id)
                return false;
            if (c.addons.length !== addons.length)
                return false;
    ...
    let modalQty = 1;
    function openAddonModal(id, name, price, img, addonsRaw, desc) {
        modalItemId = id;
        modalItemName = name;
        ...
        if (modalAddons.length > 0) {
            ...
            addonModalAddons.innerHTML = modalAddons.map((a) => `<button class="modal__addon-pill" data-addon-name="${a.name}" data-addon-price="${a.price}" type="button">
    ...
    addonModalAddons.addEventListener("click", (e) => {
        const pill = e.target.closest(".modal__addon-pill");
        if (!pill)
            return;
    ...
    addonQtyMinus.addEventListener("click", () => {
        if (modalQty > 1) {
            modalQty--;
            addonQtyNum.textContent = String(modalQty);
            updateModalTotal();
        }
    });
    ...
    /* ── Carrinho — CRUD ─────────────────────────────────── */
    function addToCart(id, name, price, img, addons, qty) {
    ...
    function renderCardButtons() {
        document.querySelectorAll(".card__order").forEach((wrap) => {
            const id = wrap.dataset.id;
            const qtyEl = wrap.querySelector(".card__qty");
            const numEl = wrap.querySelector(".card__qty-num");
    ...
    function renderDrinkButtons() {
        document.querySelectorAll(".drinks__list li[data-id]").forEach((li) => {
            const id = li.dataset.id;
            const existing = li.querySelector(".drink-qty");
            const btn = li.querySelector(".btn-drink-add");
            ...
            <button class="drink-qty-btn drink-qty-plus" data-cart-drink-plus="${id}" aria-label="Aumentar">+</button>`;
                    ...
                    const numEl = existing.querySelector(".drink-qty-num");
                    numEl.textContent = String(totalInCart);
                }
            }
            else {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (addBtn) {
            const wrap = addBtn.closest(".card__order");
            if (wrap) {
                e.preventDefault();
                openAddonModal(wrap.dataset.id, wrap.dataset.name, Number(wrap.dataset.price) || 0, wrap.dataset.img || "", wrap.dataset.addons || "", ((_b = (_a = wrap.closest(".card__body")) === null || _a === void 0 ? void 0 : _a.querySelector(".card__desc")) === null || _b === void 0 ? void 0 : _b.textContent) || undefined);
            }
            return;
        }
        ...
        const cardMinus = target.closest(".card__qty-minus");
        if (cardMinus) {
            const wrap = cardMinus.closest(".card__order");
            if (wrap) {
                const id = wrap.dataset.id;
        ...
        const cardPlus = target.closest(".card__qty-plus");
        if (cardPlus) {
            const wrap = cardPlus.closest(".card__order");
            if (wrap) {
                const id = wrap.dataset.id;
        ...
        // Drink qty minus
        const drinkMinus = target.closest("[data-cart-drink-minus]");
        if (drinkMinus) {
        ...
        // Drink qty plus
        const drinkPlus = target.closest("[data-cart-drink-plus]");
        if (drinkPlus) {
        ...
        // Carrinho: qty minus
        const cartMinus = target.closest(".cart-item__qty-minus");
        if (cartMinus) {
        ...
        // Carrinho: qty plus
        const cartPlus = target.closest(".cart-item__qty-plus");
        if (cartPlus) {
    ...
    function buildWhatsAppMessage(name, address, payment, notes) {
        const lines = [];
